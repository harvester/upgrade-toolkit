...
detect_upgrade()
{
  upgradeplan_obj=$(kubectl get upgradeplans.management.harvesterhci.io $HARVESTER_UPGRADEPLAN_NAME -o yaml)
...
update_managedchart_patch_file_annotations() {
  ...
  NEW_UPGRADE=$HARVESTER_UPGRADEPLAN_NAME yq e '.metadata.annotations."upgrade.harvesterhci.io/last-upgrade-name" = strenv(NEW_UPGRADE)' ${fname} -i
