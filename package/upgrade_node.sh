...
shutdown_repo_vm() {
  ...
  repo_vm_name="upgrade-repo-$HARVESTER_UPGRADEPLAN_NAME"
...
command_pre_drain() {
  ...
  upgrade-toolkit vm-live-migrate-detector "$HARVESTER_UPGRADE_NODE_NAME" --shutdown --upgrade "$HARVESTER_UPGRADEPLAN_NAME"
...
start_repo_vm() {
  repo_vm=$(kubectl get vm -l "harvesterhci.io/upgrade=$HARVESTER_UPGRADEPLAN_NAME" -n harvester-system -o yaml | yq -e e '.items[0].metadata.name' -)
  if [ -z $repo_vm ]; then
