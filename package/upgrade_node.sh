...
get_running_vm_count() {
  ...
  if [[ ${exit_code} -eq 0 ]]; then
    local count
    count=$(yq e '.items | map(select(.status.phase!="Succeeded")) | length' "${vmioutput}")
    echo "${count}"
  else
    echo "failed to get"
  fi
...
wait_evacuation_pdb_gone() {
  ...
  until ! kubectl get pdb -o name -A | grep -q kubevirt-migration-pdb-kubevirt-evacuation-; do
...
set_reserved_resource() {
  local cores
  cores=$(chroot "${HOST_DIR}" nproc)

  local reserved_cpu
  reserved_cpu=$(calculate_cpu_reserved_in_milli_cpu "${cores}" "${MAX_PODS}")

  # make system:kube cpu reservation ratio 2:3
  local system_reserved_cpu=$(( reserved_cpu * 2 * 2 / 5 ))
  local kube_reserved_cpu=$(( reserved_cpu * 2 * 3 / 5 ))
  local system_reserver_config="${HOST_DIR}/etc/rancher/rke2/config.yaml.d/99-z00-harvester-reserved-resources.yaml"

  if [[ ! -e "${system_reserver_config}" ]]; then
    cat > "${system_reserver_config}" << EOF
kubelet-arg+:
- "system-reserved=cpu=${system_reserved_cpu}m"
- "kube-reserved=cpu=${kube_reserved_cpu}m"
EOF
  fi
}

set_rke2_device_permissions() {
  local rke2_device_permission_config="${HOST_DIR}/etc/rancher/rke2/config.yaml.d/91-harvester-cdi.yaml"

  if [[ ! -e "${rke2_device_permission_config}" ]]; then
    cat > "${rke2_device_permission_config}" << EOF
...
calculate_cpu_reserved_in_milli_cpu() {
  local cores=$1
  local max_pods=$2

  # This shouldn't happen
  if (( cores <= 0 || max_pods <= 0 )); then
    echo 0
    return
  fi
...
cleanup_incomplete_state_file() {
  ...
  local incomplete_state_file=0

  if [[ -f "${state_file}" ]]; then
    local found
    found=$(yq '.state| has("label")' "${state_file}")
    [[ "${found}" == "false" ]] && incomplete_state_file=1
  fi

  if [[ ${incomplete_state_file} -eq 1 ]]; then
...
sparsify_passive_img() {
  ...
  local passive_img="${STATE_DIR}/cOS/passive.img"
...
check_and_mount_state() {
  local mapper_in_use
  mapper_in_use=$(chroot "${HOST_DIR}" blkid -L COS_STATE | grep mapper || true)
...
upgrade_os() {
  ...
  local tmp_elemental_config_dir
...
EOF
  ...
  local elemental_upgrade_log="${UPGRADE_TMP_DIR#"${HOST_DIR}"}/elemental-upgrade-$(date +%Y%m%d%H%M%S).log"
  local ret=0
  ...
  local grubenv_file="/oem/grubenv"
  ...
  local multipath_enabled
  multipath_enabled=$(yq '.os.externalstorage.enabled // false' "${HOST_DIR}/oem/harvester.config")

  if [[ ${multipath_enabled} == false ]]; then
    local third_party_args
    third_party_args=$(chroot "${HOST_DIR}" grub2-editenv /oem/grubenv list | grep third_party_kernel_args | awk -F"third_party_kernel_args=" '{print $2}')
    ...
    if [[ ${third_party_args} != *"multipath"* ]]; then
      third_party_args="${third_party_args} multipath=off"
      third_party_args=$(echo "${third_party_args}" | xargs)
      chroot "${HOST_DIR}" grub2-editenv /oem/grubenv set "third_party_kernel_args=${third_party_args}"
    fi
...
main() {
  ...
  case "${1:-}" in
    ...
    single-node-upgrade)
      ...
      ;;
    *)
      echo "Usage: $0 {prepare|pre-drain|post-drain|single-node-upgrade}"
      exit 1
      ;;
  esac
}
