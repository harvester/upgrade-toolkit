...
pre_upgrade_manifest() {
  if [ -e "/usr/local/share/migrations/upgrade_manifests/${UPGRADEPLAN_PREVIOUS_VERSION}/pre-hook.sh" ]; then
    echo "Executing ${UPGRADEPLAN_PREVIOUS_VERSION} pre-hook..."
    # Use source to pass current shell's variables to target script
    source "/usr/local/share/migrations/upgrade_manifests/${UPGRADEPLAN_PREVIOUS_VERSION}/pre-hook.sh"
  fi
...
preserve_overcommit_config() {
  if [[ ! "$UPGRADEPLAN_PREVIOUS_VERSION" =~ ^v1\.7\.[0-9]$ ]]; then
    echo "Skip migrate overcommit config if you are not upgrade from v1.7.x, current version: $UPGRADEPLAN_PREVIOUS_VERSION"
    return
  fi
...
clean_capi_legacy_webhooks() {
  ...
  echo "clean capi legacy webhooks which are not used from Harvester v1.8.0"
...
EOF
  ...
  upgrade_managed_chart_from_version $UPGRADEPLAN_PREVIOUS_VERSION harvester ${hpatch}
  ...
  if [ -n "$sc" ] && [ "$UPGRADEPLAN_PREVIOUS_VERSION" != "v1.0.3" ]; then
...
apply_extra_manifests()
{
  ...
  if [[ $(is_formal_release $UPGRADEPLAN_PREVIOUS_VERSION) = "true" ]] && [[ "$UPGRADEPLAN_PREVIOUS_VERSION" < "v1.1.2" ]]; then
    local rootpath="/usr/local/share/extra_manifests/untilv1.1.1"
  else
    local rootpath="/usr/local/share/extra_manifests/$UPGRADEPLAN_PREVIOUS_VERSION"
  fi
...
apply_tls_internal_cn_allowed_services_setting() {
  if [[ ! "$UPGRADEPLAN_PREVIOUS_VERSION" =~ ^v1\.8[.-][a-zA-Z0-9.-]+$ ]]; then
    echo "Skip set tls-internal-cn-allowed-services if you are not upgrade from v1.8.x, current version: $UPGRADEPLAN_PREVIOUS_VERSION"
    return
  fi
